...
extension IDLAttribute: SwiftRepresentable, Initializable {
    ...
    var swiftRepresentation: SwiftSource {
        ...
        if Context.override {
            ...
            set { \(idlType.propertyWrapper(readonly: readonly))[Keys.\(name: name), in: jsObject] = newValue }
            """

            return """
            public var \(name: name): \(idlType) {
                get { \(idlType.propertyWrapper(readonly: readonly))[Keys.\(name: name), in: jsObject] }
                \(readonly ? "" : setter)
            }
            """
        } else {
    ...
    var initializer: SwiftSource? {
        assert(!Context.static)
        return """
        \(wrapperName) = \(idlType.propertyWrapper(readonly: readonly))(jsObject: jsObject, name: Keys.\(name: name))
        """
    }
}
...
extension MergedDictionary: SwiftRepresentable {
    ...
    private var swiftInit: SwiftSource {
        ...
        public convenience init(\(sequence: params)) {
            ...
            \(lines: membersWithPropertyWrapper.map { member, wrapper in
                if member.idlType.isFunction {
                    return """
                    \(wrapper)[Keys.\(name: member.name), in: object] = \(name: member.name)
                    """
                } else {
                    return """
                    object[Keys.\(name: member.name)] = \(name: member.name).jsValue()
                    """
                }
            })
        ...
        public required init(unsafelyWrapping object: JSObject) {
            \(lines: membersWithPropertyWrapper.map { member, wrapper in
                "_\(raw: member.name) = \(wrapper)(jsObject: object, name: Keys.\(name: member.name))"
            })
...
extension IDLOperation: SwiftRepresentable, Initializable {
    ...
    fileprivate var defaultBody: (prep: SwiftSource, call: SwiftSource) {
        ...
        return (
            prep: "\(lines: prep)",
            call: "\(Context.this)[Keys.\(name: name)]!(\(sequence: args))"
        )
    }
