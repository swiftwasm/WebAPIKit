...
struct UnionProtocol: SwiftRepresentable {
    ...
    var swiftRepresentation: SwiftSource {
        """
        public protocol Any_\(types.inlineTypeName): ConvertibleToJSValue {}
        \(lines: extensions)

        public enum \(types.inlineTypeName): JSValueCompatible, Any_\(types.inlineTypeName) {
            \(lines: cases)
    ...
    var extensions: [SwiftSource] {
        sortedTypes.map {
            "extension \($0.typeName): Any_\(types.inlineTypeName) \($0.whereClause) {}"
        }
    }
